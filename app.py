...
def load_model():
    ...
    # Load checkpoint
    ckpt = torch.load(temp_path, map_location=device, weights_only=False)
...
@torch.no_grad()
def reconstruct_uploaded(model, noise_schedule, uploaded_img, start_t_val, ddim_steps_val):
...
st.markdown("Pure noise se generate karo **ya** apni image reconstruct karo!")
        ...
        if st.button("Generate!", type="primary"):
            with st.spinner("Generating images..."):
                ...
                for i in range(num_images):
                    arr = (denormalize(gen_imgs[i].cpu()).permute(1,2,0).numpy() * 255).astype(np.uint8)
                    st.image(arr, caption=f"Generated Image {i+1}", use_container_width=True)
                
                # Display denoising steps
                if intermediates:
                    st.subheader("Denoising Progress")
                    cols = st.columns(min(len(intermediates), 5))
                    for idx, img in enumerate(intermediates):
                        if idx < len(cols):
                            cols[idx].image((img*255).astype(np.uint8), caption=f"Step {idx+1}", use_container_width=True)

with tab2:
    col1, col2 = st.columns([1, 1])
    
    with col1:
        uploaded_file = st.file_uploader("Apni image upload karo", type=['png', 'jpg', 'jpeg'])
        noise_level = st.slider("Noise Level", 50, 350, 250, step=50)
        ddim_steps_rec = st.slider("DDIM Steps", 10, 100, 50, step=10, key="rec_steps")
        
        if uploaded_file is not None and st.button("Reconstruct!", type="primary"):
            img = PILImage.open(uploaded_file).convert('RGB')
            with st.spinner("Reconstructing..."):
                original, noisy, reconstructed = reconstruct_uploaded(
                    model, noise_schedule, img, noise_level, ddim_steps_rec
                )
                
                with col2:
                    st.image(original, caption="Original", use_container_width=True)
                    st.image(noisy, caption="Noisy", use_container_width=True)
